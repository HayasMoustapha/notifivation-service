...
/**
 * Middleware de vérification de clé API (pour les webhooks)
 * @param {string} keyName - Nom de la clé API attendue
 */
function requireAPIKey(keyName = 'API_KEY') {
  return (req, res, next) => {
    ...
    if (!apiKey || apiKey !== expectedKey) {
      ...
      return res.status(401).json(
        errorResponse('Clé API invalide', null, 'INVALID_API_KEY')
      );
    }
...
/**
 * Middleware de vérification de webhook secret
 * @param {string} secretName - Nom du secret attendu
 */
function requireWebhookSecret(secretName = 'WEBHOOK_SECRET') {
  return (req, res, next) => {
    ...
    if (!signature || !expectedSecret) {
      return res.status(401).json(
        errorResponse('Signature de webhook requise', null, 'MISSING_WEBHOOK_SIGNATURE')
      );
    }
    ...
    if (signature !== expectedSignature) {
      ...
      return res.status(401).json(
        errorResponse('Signature de webhook invalide', null, 'INVALID_WEBHOOK_SIGNATURE')
      );
    }

    logger.auth('Webhook signature validated', {
      secretName,
      ip: req.ip
    });
